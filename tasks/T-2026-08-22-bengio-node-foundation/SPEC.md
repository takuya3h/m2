# 基盤を作り、中心宛の鍵と識別子を公開する（bengio）

**task_id:** `T-2026-08-22-bengio-node-foundation`  **kind:** `impl`
...
1. **中心宛の鍵の公開鍵** → `scripts/sync/hub_keys/bengio.pub`
2. **自ホストの識別子** → `scripts/sync/device_ids/bengio.txt`
...
**取り込みは手で行う。最初に版管理を最新にすること。**
    ...
    git fetch origin
    git checkout -b feat/bengio-node-foundation origin/phase0
    git --no-pager log -1 --format='%h %s'
    mkdir -p tasks/T-2026-08-22-bengio-node-foundation
...
**Files:** Create: `tasks/T-2026-08-22-bengio-node-foundation/audit.md`
...
**Files:** Create: `~/.ssh/id_ed25519_bengiotophilip`,
...
**合言葉は付けない。** 常駐処理が対話なしで使うためである。
    ...
    ssh-keygen -t ed25519 -N "" -C "bengiotophilip" \
      -f ~/.ssh/id_ed25519_bengiotophilip
...
- [ ] **Step 3: 権限を確かめる**
    ...
    stat -c "%a %n" ~/.ssh/id_ed25519_bengiotophilip ~/.ssh/id_ed25519_bengiotophilip.pub
    stat -c "%a %n" ~/.ssh
...
**Files:** Create: `~/bin/syncthing`, `scripts/sync/device_ids/bengio.txt`

- [ ] **Step 1: 配布物を取得し、照合する**

    mkdir -p ~/bin
    cd /tmp && curl -sSL -o st_bengio.tar.gz \
...
- [ ] **Step 2: 展開して配置する**
    ...
    cd /tmp && tar xzf st_bengio.tar.gz
...
**Files:** Create: `tasks/T-2026-08-22-bengio-node-foundation/RESULT.md`, `tasks/T-2026-08-22-bengio-node-foundation/result.yaml`,
...
- [ ] **Step 2: 送信前に自分で秘匿を検査する**
    ...
    grep -n -i -E "BEGIN [A-Z ]*PRIVATE|api[_-]?key|password|passphrase" \
      tasks/T-2026-08-22-bengio-node-foundation/*.md tasks/T-2026-08-22-bengio-node-foundation/*.yaml 2>&1
...
- [ ] **Step 3: 検証を通す**
    ...
    source .venv/bin/activate \
      && make task-validate TASK=T-2026-08-22-bengio-node-foundation; echo "validate_exit=$?"
...
- [ ] **Step 5: 変更範囲と未追跡を確かめる**
    ...
    git --no-pager status --porcelain > /tmp/hf_bengio.txt
    grep -c '' /tmp/hf_bengio.txt; cat /tmp/hf_bengio.txt
...
- [ ] **Step 6: commit し、送出する**
    ...
    git add tasks/T-2026-08-22-bengio-node-foundation/ tasks/inbox.d/T-2026-08-22-bengio-node-foundation.md \
            scripts/sync/hub_keys/bengio.pub \
            scripts/sync/device_ids/bengio.txt \
            context/auto/ tasks/inbox.md
    git commit -m "feat(sync): build foundation and publish hub key and device id on bengio"
    git --no-pager log -1 --format='%h %s'
