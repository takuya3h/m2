...
### 独立した敵対的レビューで見つかった欠陥と、その修正

上記までを終えた段階で、**「どんな失敗の仕方をしても痕跡を残さない」という主張を反証させる**目的で
独立したレビューを実施した。結果、**主張は偽であった**。指摘は受け売りにせず、
一つずつ自分で再現してから修正し、修正後に再現手順で再確認した。

| # | 欠陥 | 再現の実測 | 修正 |
|---|---|---|---|
| 1 | **task_id の検証をすり抜ける経路があった（最も重大）** | `_TASK_ID_RE` の終端が `$` だった。Python の `$` は**末尾の改行の直前にも一致する**ため、`task_id: "T-2026-08-03-valid\n"` が通ることを実測。改行入りの値は `make` のレシピを分断し、**別の既存契約を検証させたうえで exit 0 を返させる**（検証バイパス） | 終端を `\Z` へ変更。再現手順で拒否されることを確認 |
| 2 | **巻き戻しが「自分が作っていないディレクトリ」を消していた（残骸ではなくデータ損失）** | `entered = True` を `copytree` の**前**に立てていたため、`ensure_absent` と `copytree` の間に同名ディレクトリが出来ると `FileExistsError` → 既存ディレクトリを `rmtree`。probe の `IMPORTANT.md` が**実際に消えた**うえ「痕跡は残していません」と表示された | `installed.mkdir()` で名前をアトミックに確保し、成功した場合のみ所有権を立てる。既存なら `FileExistsError` を捕まえて中止し、**触らない**。再現手順で既存データが残ることを確認 |
| 3 | **Ctrl-C や SIGTERM で設置が残っていた** | `except Exception` は `KeyboardInterrupt` と `SystemExit`（`BaseException` 由来）を捕まえない。検証は `make` の子プロセスであり、中断は現実的な経路 | `except BaseException` へ変更し、SIGTERM を `SystemExit` へ変換するハンドラを追加。巻き戻したうえで再送出する |
| 4 | **消し切れなくても「痕跡は残していません」と書いていた** | `shutil.rmtree(..., ignore_errors=True)` の結果を見ずに無条件でその文言を出していた | `rollback()` を切り出し、**消えたことを確かめてから**文言を出す。消し切れなければ残った場所をそのまま報告する |
| 5 | SIGKILL 中の一時ディレクトリ残留 | `TemporaryDirectory` の後片付けが走らない | **修正しない。** ユーザー空間で防げない。§10 に既知の限界として申し送る |

修正はいずれもテストで固定した（この 5 件に対して 6 件を追加）。
なお 4 と同型の「確かめずに断定する」誤りは、本 RESULT の §3 でも一度起こしている
（`pgrep -f` の偽陽性）。**検査そのものを検査する**必要があることが二度示された。

レビューが「欠陥ではない」と確認した点も記録する。`ALLOWED_FILES` は 3 要素の完全一致であり、
ファイル名で staging の外へ出る経路は無い。`pack_bundle` は許可外の名前を出力できない。
...
| `tests/test_fetch_task.py` | **22 passed**（新規。うち 6 件は敵対的レビューで見つかった欠陥を固定するもの） |
| 上記 2 ファイル合計 | **31 passed** |
| 全体 `tests/` | **5 failed / 213 passed**。失敗は実行前と同じ 5 件で不変。passed が 189 から 213 へ増えた 24 件は本 task が追加した分（2 + 22）と一致する |
...
| 10 | テストが全 pass | PASS（**31 passed**） |
| 11 | 全体テストが不変 | PASS（**5 failed / 213 passed**。失敗 5 件で不変） |
...
- 指示: SPEC は独立したレビューを要求していない。
- 実際: Phase C を終えた段階で、「どんな失敗の仕方をしても痕跡を残さない」という主張を**反証させる**ための独立した敵対的レビューを実施し、見つかった 4 件の欠陥（検証バイパス・データ損失・中断時の残骸・確かめずに断定）を修正して commit を 1 本追加した。
- 理由: 自分で書いた実装を自分で確かめるだけでは、`except Exception` が `KeyboardInterrupt` を捕まえないことや `$` が末尾改行に一致することのような盲点は残る。実際に**データ損失を起こす欠陥**が見つかっており、実施しなければそのまま PR に載っていた。
- 分類: 判断が必要だった
...
- **SIGKILL 中の一時ディレクトリ残留は防げない。** `SIGKILL` はユーザー空間で捕まえられないため、staging 中に受けると `/tmp/.task_fetch_*` が残る。実測でも再現している（本 task の検証中に生じた 1 件は削除済み）。`SIGINT` と `SIGTERM` は巻き戻しの経路に載せたが、`SIGKILL` は**既知の限界**として残す。定期的な `/tmp` の掃除に委ねる。
- **巻き戻しに失敗した場合は人手が要る。** `rollback()` は消えたことを確かめ、消し切れなければ残った場所を報告して終わる。この状態（例えば `tasks/` が書けない）では `make task-validate` が以後 FAIL し続けるため、報告された場所を人が処理する必要がある。自動での再試行は実装していない。
- **バンドルの最後のファイル本文に区切りと `END` を意図的に埋め込むと、そのファイルが静かに切り詰められる。** 形式上、区切り + `END` が終端の定義そのものであるため解析側では正規の終端と区別できない。ただし切り詰められた `spec.yaml` / `SPEC.md` は後段の L1・L2 検証で落ち、巻き戻される。**設計上の受容**として記録する。
