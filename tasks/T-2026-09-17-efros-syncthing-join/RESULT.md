...
| AA | repo の進み方 | 16:43:34 `scanning` local 17654225337 → 16:45:41 `sync-preparing` local 44514451803。**完了していない** |
...
| repo の同期 | 2 分間で `localBytes` が 17.65 → 44.51 GB。**ただしこれは走査であって受信ではない**（同区間の `du -sb` の増分は **834 bytes**） |
...
- **repo `m2` の同期が完了する時刻。** 完了を待たない指示のため測っていない。
