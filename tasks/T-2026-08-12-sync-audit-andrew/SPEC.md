# 設定同期の停止に関する実測（andrew）

**task_id:** `T-2026-08-12-sync-audit-andrew`  **kind:** `analysis`  **depends_on:** なし
...
`tasks/inbox.d/T-2026-08-12-sync-audit-andrew.md` へ置く。集約は後日まとめて行う。
...
| 8 | 出力は要約せず `tasks/T-2026-08-12-sync-audit-andrew/audit.md` へ貼る |
...
**Files:** Create: `tasks/T-2026-08-12-sync-audit-andrew/probe.py`, `tasks/T-2026-08-12-sync-audit-andrew/audit.md`
...
**Files:** Modify: `tasks/T-2026-08-12-sync-audit-andrew/audit.md`
...
- [ ] **Step 3: 中継の入口へ接続する。** ポートは Task 2 Step 3 で読んだ値を使う。
    ...
    .venv/bin/python tasks/T-2026-08-12-sync-audit-andrew/probe.py 127.0.0.1:PORT
...
ポート番号は実装と設定から得た値を使う。記録から決め打ちしない。
    ...
    .venv/bin/python tasks/T-2026-08-12-sync-audit-andrew/probe.py ADDR:PORTA ADDR:PORTB > /tmp/reach.txt
    wc -l /tmp/reach.txt
    cat /tmp/reach.txt
...
**Files:** Modify: `tasks/T-2026-08-12-sync-audit-andrew/audit.md`, Create: `tasks/T-2026-08-12-sync-audit-andrew/inventory.tsv`
...
- [ ] **Step 2: 一覧に要約値を付ける**
    ...
    wc -l /tmp/inv.tsv
    .venv/bin/python - <<'PY' > tasks/T-2026-08-12-sync-audit-andrew/inventory.tsv
    ...
    PY
    wc -l tasks/T-2026-08-12-sync-audit-andrew/inventory.tsv
...
内容を含まないことを明記する。
    ...
    grep -c -i -E "apikey|password|token|secret|PRIVATE KEY" tasks/T-2026-08-12-sync-audit-andrew/inventory.tsv
...
- [ ] **Step 4: 復旧時に消えうるものと更新時刻の分布を記録する**
    ...
    sort -t"	" -k3 tasks/T-2026-08-12-sync-audit-andrew/inventory.tsv > /tmp/sorted.tsv
...
**Files:** Create: `tasks/T-2026-08-12-sync-audit-andrew/RESULT.md`, `tasks/T-2026-08-12-sync-audit-andrew/result.yaml`,
`tasks/inbox.d/T-2026-08-12-sync-audit-andrew.md`
...
単体の値と食い違っても異常と断定しない。両方を記録する。
    ...
    make task-validate TASK=T-2026-08-12-sync-audit-andrew; echo "validate_exit=$?"
    make task-preflight TASK=T-2026-08-12-sync-audit-andrew; echo "preflight_exit=$?"

- [ ] **Step 4: 判断の受け皿へ置く。** `tasks/inbox.d/T-2026-08-12-sync-audit-andrew.md` に
...
- [ ] **Step 6: commit する**
    ...
    git add tasks/T-2026-08-12-sync-audit-andrew/ tasks/inbox.d/T-2026-08-12-sync-audit-andrew.md
    git commit -m "docs(sync): audit sync topology and divergence on andrew"
    git --no-pager log -1 --format='%h %s'

- [ ] **Step 7: 抑止を解除する（削除ではなく移動）**

    mv .sync-pause /tmp/.sync-pause.released.T-2026-08-12-sync-audit-andrew 2>/dev/null \
      && echo "released" || echo "解除に失敗。手当てが要る"
    ls -la .sync-pause 2>/dev/null && echo "まだ残っている" || echo "repo 直下から消えた"

- [ ] **Step 8: 報告を台帳へ返す**

    make task-report TASK=T-2026-08-12-sync-audit-andrew; echo "exit=$?"
