# 中継用の公開鍵を提出し、中心への到達住所を測る（andrew）
...
四つ目について、**契約自身のディレクトリ `tasks/T-2026-08-12-submit-hub-key-andrew/` は `task-start` が
...
**Files:** Create: `tasks/T-2026-08-12-submit-hub-key-andrew/audit.md`
...
- [ ] **Step 5: 秘匿が混ざっていないことを確かめる**
    ...
    grep -n -i -E "BEGIN [A-Z ]*PRIVATE|api[_-]?key|password|passphrase|secret" tasks/T-2026-08-12-submit-hub-key-andrew/audit.md
...
**Files:** Modify: `tasks/T-2026-08-12-submit-hub-key-andrew/audit.md`
...
**Files:** Create: `scripts/sync/hub_keys/andrew.pub`
...
- [ ] **Step 2: 公開鍵を置く**
    ...
    cp /tmp/pub_derived.txt scripts/sync/hub_keys/andrew.pub
    wc -c scripts/sync/hub_keys/andrew.pub
...
- [ ] **Step 3: 置いたものが公開鍵だけであることを確かめる**
    ...
    head -c 40 scripts/sync/hub_keys/andrew.pub; echo
    grep -c "PRIVATE" scripts/sync/hub_keys/andrew.pub
    wc -l scripts/sync/hub_keys/andrew.pub
...
`tasks/T-2026-08-12-submit-hub-key-andrew/handoff.md` に次を記す。
...
**Files:** Create: `tasks/T-2026-08-12-submit-hub-key-andrew/RESULT.md`, `tasks/T-2026-08-12-submit-hub-key-andrew/result.yaml`,
`tasks/inbox.d/T-2026-08-12-submit-hub-key-andrew.md`
...
- [ ] **Step 3: 検証を通す**
    ...
    make task-validate TASK=T-2026-08-12-submit-hub-key-andrew; echo "validate_exit=$?"
    make task-preflight TASK=T-2026-08-12-submit-hub-key-andrew; echo "preflight_exit=$?"
...
`tasks/inbox.d/T-2026-08-12-submit-hub-key-andrew.md` に**起票者が次の判断に使える事実だけ**を置く。
...
- [ ] **Step 7: commit する**
    ...
    git add scripts/sync/hub_keys/andrew.pub tasks/T-2026-08-12-submit-hub-key-andrew/ tasks/inbox.d/T-2026-08-12-submit-hub-key-andrew.md
    git commit -m "feat(sync): submit tunnel public key for andrew"
...
- [ ] **Step 9: 抑止を解除し、報告を台帳へ返す**
    ...
    mv .sync-pause /tmp/.sync-pause.released.T-2026-08-12-submit-hub-key-andrew 2>/dev/null \
      && echo "released" || echo "解除に失敗。手当てが要る"
    ls -la .sync-pause 2>/dev/null && echo "まだ残っている" || echo "repo 直下から消えた"
    make task-report TASK=T-2026-08-12-submit-hub-key-andrew; echo "exit=$?"
