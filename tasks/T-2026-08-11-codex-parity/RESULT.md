...
Codex で、契約の検証、シェル差の実測、手順書修正、機械検査の追加まで実行した。
この版では PR 起票と台帳返却（G3）が未実施であるため、完了判定は `partial` とする。
...
PR起票、台帳返却、同期抑止解除はこの報告を版管理へ記録した後に行うため、現時点では未測定。
...
未測定値は書いていない。G3の結果は実行後にこの報告と `result.yaml` の両方へ追記する。
