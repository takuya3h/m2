...
## Task 3 追記: 契約の実行中に本ホストの状態が変わった

**報告の直前に家の直下を数え直したところ 29 → 38 に増えていた。**
**実行者が作ったのは控えの 1 件だけである。残り 8 件は実行者の操作によるものではない。**

```
$ Task 1 の一覧と現在の一覧を comm で対照
=== 増えたもの ===
.agents
.cargo
.codex
...
.rustup
.zsh
adam-preserve-2026-08-25   ← 実行者が作った控え
setup_server.sh
=== 減ったもの ===
(空。減少なし)
```
...
```
$ 新規 8 件の時刻と大きさ
.agents              2026-08-23 18:11:56        26003547 bytes
.cargo               2026-08-25 13:59:12        20839218 bytes
.codex               2026-08-25 14:01:03              57 bytes
.npm                 2026-08-25 13:59:25       527053581 bytes
.nvm                 2026-08-25 13:59:10      1995691144 bytes
.rustup              2026-08-25 13:59:13      1356927811 bytes
.zsh                 2026-08-25 13:59:48            1701 bytes
setup_server.sh      2026-08-25 13:57:59           17922 bytes

$ head ~/setup_server.sh
#!/usr/bin/env bash
# ============================================================================
# setup_server.sh
#
# 全サーバー共通の一括セットアップスクリプト（ホスト非依存）。
# setup_andrew.sh（最新版）をベースに汎用化し、以下の 4 項目を除外している。
#
#   除外: claude-pulse / semgrep / failog / gemini (@google/gemini-cli)
```

🔴 **本ホストへ再構築の手順が適用されている最中である。**
`setup_server.sh`（全サーバー共通の一括セットアップ）が 13:57:59 に置かれ、
13:59 以降に nvm / npm / rustup / cargo / codex / zsh が導入された。
**契約が前提に置いた「本ホストはその対象外であり」は、契約の実行中に成り立たなくなった。**

**減ったものは無い。** 読み取りのみという契約の性質は保たれている。
`~/.codex/` は Task 1 で「無し」と記録したが、**現在は在る**（57 bytes）。
`~/bin/` `~/.local/state/syncthing/` `~/claude-sync/` は依然として無い。

**この変化は実行者の操作ではないため逸脱ではない。想定外として記録する。**
**Task 1 の家の直下の計数は 2026-08-25 の測定時点の値であり、定数として扱えない。**
