...
**task_id:** `T-2026-08-12-tunnel-key-audit-andrew`  **kind:** `analysis`  **depends_on:** `T-2026-08-12-sync-audit-andrew`
...
| 8 | 出力は要約せず `tasks/T-2026-08-12-tunnel-key-audit-andrew/audit.md` へ貼る |
...
**Files:** Create: `tasks/T-2026-08-12-tunnel-key-audit-andrew/audit.md`
...
- [ ] **Step 4: 秘匿が混ざっていないことを確かめる**
    ...
    grep -c -i -E "BEGIN [A-Z ]*PRIVATE|api[_-]?key|password|passphrase|secret" tasks/T-2026-08-12-tunnel-key-audit-andrew/audit.md
...
**Files:** Modify: `tasks/T-2026-08-12-tunnel-key-audit-andrew/audit.md`
...
**Files:** Create: `tasks/T-2026-08-12-tunnel-key-audit-andrew/RESULT.md`, `tasks/T-2026-08-12-tunnel-key-audit-andrew/result.yaml`,
...
- [ ] **Step 3: 検証を通す。** `make` 経由の終了コードはレシピ失敗時に `2` になる。
    ...
    make task-validate TASK=T-2026-08-12-tunnel-key-audit-andrew; echo "validate_exit=$?"
    make task-preflight TASK=T-2026-08-12-tunnel-key-audit-andrew; echo "preflight_exit=$?"

**`P9` の `host_mismatch` は既知の偽陽性である。** 切り分けを繰り返さない。

- [ ] **Step 4: 判断の受け皿へ置く。** `tasks/inbox.d/T-2026-08-12-tunnel-key-audit-andrew.md` に
...
- [ ] **Step 6: 送信前の自己検査（道具の欠陥を避ける）**
    ...
    for f in ["RESULT.md", "result.yaml", "audit.md"]:
        p = pathlib.Path("tasks/T-2026-08-12-tunnel-key-audit-andrew") / f
...
- [ ] **Step 7: commit する**
    ...
    git add tasks/T-2026-08-12-tunnel-key-audit-andrew/ tasks/inbox.d/T-2026-08-12-tunnel-key-audit-andrew.md
    git commit -m "docs(sync): audit tunnel key distribution on andrew"
    git --no-pager log -1 --format='%h %s'

- [ ] **Step 8: 抑止を解除する（削除ではなく移動）**

    mv .sync-pause /tmp/.sync-pause.released.T-2026-08-12-tunnel-key-audit-andrew 2>/dev/null \
