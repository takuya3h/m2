...
証跡は `audit.md`。以下は節番号で指す。手順書は `handoff.md`。

**時刻は JST**（記録の原文を引用した箇所だけ UTC。同期処理が UTC で書くため。JST = UTC + 9 時間）。
...
`urAccepted` は 2026-09-17 14:07 JST の控えで `0`、現行（2026-09-21 02:32:39 JST）で `3`。
初回送信は **2026-09-21 02:32:36 JST**。**28 日間 0 件だったものが日次 1 件になった。**
...
1. 使用状況の送信が止まったか — 次に送るはずの **2026-09-23 02:32 JST** を越えていない（8.7）
