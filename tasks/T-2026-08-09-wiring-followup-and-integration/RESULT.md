...
| 6 | 資格情報の失効が未設定と区別されない | ✅ 検出を追加。**401 は実環境で検証済み。通過経路は未検証**（§5-3） |

**あわせて前 task の結論を 1 点訂正した。** 下書きの起票には経路が 2 つあり、
ワークフローは失敗しているが常駐スクリプトは成功している（§5-4）。
...
**401 の経路は実環境でも確認できた。** 本 task の commit が push された際に発火した
GitHub Actions の実行（run `31223408692` / 2026-08-07T22:19:33Z）で、
実際に次の出力が得られている。

```
##[error]secret AUTOSYNC_PR_TOKEN は設定されているが無効。失効・取り消し・値の誤りのいずれか。
         fine-grained PAT を再発行して secret を更新してください（README §5.5 Step 3）。
```

同じ資格情報に対して、変更前は「secret AUTOSYNC_PR_TOKEN が未設定」と出ていた。
**誤誘導は解消された。**
...
| 経路 | 状態 |
|---|---|
| 401（失効） | **実環境で検証済み**（run `31223408692`） |
| 403（権限不足） | **未検証**。応答文字列を流した論理検証のみ |
| 404（到達不能） | **未検証**。同上 |
| 判別不能 | **未検証**。同上 |
| 200（通過） | **未検証**。有効な資格情報が無いため確認できない |

**通過経路は資格情報の再発行後に確認が必要である。「動くはず」とは書かない。**

なお `AUTOSYNC_PR_TOKEN` そのものは変更していない（利用者の操作領域）。
2026-08-07 の 21:17:20Z / 21:49:25Z / 22:19:33Z の 3 回とも 401 で失敗しており、失効は継続している。

### 5-4. 前 task の結論の訂正 — 下書きの起票には経路が 2 つある

前 task（`T-2026-08-09-run-wiring-verification`）は「③ 下書きの起票は走ったが失敗した」と報告した。
**ワークフロー経路については正しいが、「PR が起票されなかった」という含意は誤りである。**

本 task で PR を起票しようとしたところ、**PR #51 が既に存在していた**。

| 項目 | 実測 |
|---|---|
| PR | **#51** `auto: exp/lecun-wip-20260703 -> phase0`（Draft / base `phase0`） |
| 作成時刻 | **2026-08-07T21:19:16Z** |
| 本文 | `m2-sync.sh による自動起票（2026-08-07 21:19:15）。1 commits。` |

前 task で `gh pr list --head <branch> --state open` を測ったのは 21:18 頃であり、
**`m2-sync.sh` が PR を作った 21:19:15 より前だった。測定が早すぎたための誤りである。**

したがって下書きの起票には経路が 2 つある。

| 経路 | 状態 |
|---|---|
| GitHub Actions の `auto-draft-pr.yml` | **失敗し続けている**（401。本 task で文言を修正） |
| 常駐スクリプト `m2-sync.sh` | **成功している**（PR #51 を起票） |

**片方が失敗していても、もう片方が起票するため PR は存在しうる。**
ワークフローの失敗だけを見て「起票されていない」と判断してはならない。
...
### D-7. PR を新規作成せず、既存の PR #51 を更新した

- **指示:** Phase E Task 7 Step 6 の `gh pr create --base phase0 --head "$BR" --title … --body-file …`
- **実際:** 同じ head と base の PR **#51 が既に存在していた**（`m2-sync.sh` が 2026-08-07T21:19:16Z に起票）。
  同一の組で 2 本目を作ることはできないため、**#51 の題と本文を更新**した。
- **理由:** SPEC の意図は「何が全ホストへ配られるかを利用者が判断できるようにする」ことであり、
  81 文字の雛形本文ではその意図を満たさない。新規作成は不可能であり、更新が意図に最も近い。
  もとの本文にあった起票元と時刻の情報は、更新後の本文の冒頭に残した。
- **分類:** **SPEC の欠陥**（起票の経路が 2 つあることを想定していない。§5-4 参照）

### D-8. `git push` は実行したが、既に送出済みだった

- **指示:** Phase E Task 7 Step 6 の `git push origin "$BR"`
- **実際:** 実行したが `Everything up-to-date` だった。常駐スクリプトが先に自動 push していたため。
  `origin/exp/lecun-wip-20260703` と `HEAD` はいずれも `04dab86` で一致している。
- **分類:** **環境差**（記録のため列挙。指示に反してはいない）
...
§5-3 のとおり。401 の経路は実環境で確認できたが、**有効な資格情報が無いため 200 の経路は
確認できていない。** 403 / 404 / 判別不能の分岐も論理検証にとどまる。
`AUTOSYNC_PR_TOKEN` の再発行は利用者の操作領域であり、本 task では触れていない。
再発行後に、下書きの起票が実際に成功することの確認が必要である。

なお `m2-sync.sh` 経由の起票は成功しているため（§5-4）、**PR が作られないという実害は
現時点で発生していない。** 直すべきは「失敗し続けるワークフローが毎 push でノイズを出すこと」と
「経路が二重であること」であり、優先度の判断が要る。
