...
## 容量・epoch・解像度の実測（報告後の追記）

### 時間ヘッドの容量

`maps` は 64、段数は 2 で固定。層数だけが掃引される。パラメータ数はモデルを組んで数えた。

| 構成 | パラメータ数 | 受容野 |
|---|---|---|
| 候補 A / C・6 層（**確定塔**） | 331,090 | 253 フレーム |
| 候補 A / C・8 層 | 397,138 | 1021 フレーム |
| 候補 B・6 層 | 382,299 | 253 フレーム |
| 候補 B・8 層 | 448,347 | 1021 フレーム |
| 単フレーム線形対照（一周目） | 18,441 | 1 フレーム |

候補 A と C はパラメータ数が同一である（C は損失に平滑化項を足すだけで構造は A と同じ）。
候補 B は A の上に Transformer 集約（4 head・1 層・FFN 256・dropout 0.5・位置符号なし）を
載せるため 51,209 多い。

**fine-tune した backbone の学習対象は 23,516,937 パラメータ**（layer1 215,808 + layer2 1,219,584 +
layer3 7,098,368 + layer4 14,964,736 + fc 18,441）。時間ヘッドはその **約 1.4%** の容量である。

### 時間ヘッドの学習 epoch

上限 50、val macro Jaccard の改善なし 10 epoch で打ち切り（2 epoch warmup + cosine、
lr 5e-4、weight_decay 0.01、batch は動画単位で 1）。一周目と同一。140 run の実測は次のとおり。

| | 最小 | 最大 | 平均 |
|---|---|---|---|
| 打ち切りまでに回した epoch | 13 | 50 | 20.1 |
| 最良 epoch | 3 | 42 | 10.2 |

**上限 50 に達した run は 1 件のみ**で、ほぼ全 run が早期打ち切りで終わった。
時間ヘッド側は上限が結果を切っていない（backbone の fine-tune 側は上限 12 に張り付いた run が
2 件あり、そちらは切っている可能性が残る）。

### 入力解像度

元画像は 1920×1080。前処理は torchvision の `ResNet50_Weights.IMAGENET1K_V1.transforms()`
（`ImageClassification`）をそのまま使う。

    resize_size=[256]   短辺を 256 へ（bilinear）
    crop_size=[224]     中央 224x224 を切り出す
    mean=[0.485, 0.456, 0.406]  std=[0.229, 0.224, 0.225]

fine-tune の train 側だけ `RandomHorizontalFlip(p=0.5)` が前段に入る。
val・test と特徴抽出は増強なしで同一。**一周目の特徴抽出と同じ前処理**である。

🔴 **報告時に resize_size を 232 と書いたのは誤りで、実測は 256 である。**
`RESULT.md`・`result.yaml`・`audit.md`・`tasks/inbox.d/` の該当箇所を実測値へ直した。
数値そのものを見ずに torchvision の別の重みの既定値と取り違えた。
`conventions#issuer_cautions` の注意 4（仕組みの挙動は実装を読んでから信じる）の型である。
**前処理は一周目と同一であるという結論は変わらない**（両者とも同じ `transforms()` を呼ぶため）。

## 前処理・増強・数値精度・実効 batch の実測（報告後の追記）

### 前処理と増強

| 段階 | 前処理 | 増強 |
|---|---|---|
| fine-tune（train） | 短辺 256 へ Resize（bilinear）→ 中央 224×224 切り出し → 正規化 | **`RandomHorizontalFlip(p=0.5)` のみ** |
| fine-tune（val） | 同上 | なし |
| 特徴抽出 | 同上 | なし |
| 時間ヘッド | なし（2048 次元の特徴を読むだけ） | なし |

正規化は mean `[0.485, 0.456, 0.406]` / std `[0.229, 0.224, 0.225]`。
**増強は水平反転 1 種類だけ**で、色調・スケール・回転・切り出しのゆらぎ・時間方向の増強は入れていない。
時間ヘッドは特徴キャッシュを読むため、増強を入れる余地が構造上ない（毎 epoch 同じ特徴列を見る）。

### 数値精度

**全段階 fp32。混合精度も TF32 も使っていない。** `deterministic()` が実行プロセス内で次を立てる。

    matmul.allow_tf32  = False   （torch の既定も False）
    cudnn.allow_tf32   = False   （torch の既定は True。**能動的に落としている**）
    cudnn.deterministic = True
    cudnn.benchmark     = False
    torch.use_deterministic_algorithms(True)

実測: 既定 dtype `torch.float32`、保存した特徴の dtype `float32`、時間ヘッドの重みの dtype
`torch.float32`。`autocast` / `GradScaler` / `.half()` / `bfloat16` はコードに 0 箇所。
一周目と同一で、SPEC §1 の「数値精度は一周目と同じ（変えない）」を守っている。

`T-2026-09-17-amp-compile-timing`（PR #182）が検出側で TF32 による 1.18〜1.29 倍を実測しているが、
採用の可否は利用者判断として保留されており、本契約では触れていない。

### 実効 batch と step 数

勾配累積は使っていないため、実効 batch は下表の値そのものである。

| 段階 | batch | 1 epoch の step 数 | 1 step のフレーム数 |
|---|---|---|---|
| fine-tune | **64 フレーム** | 151〜175 | 64（最終 step のみ端数） |
| 特徴抽出 | 32 フレーム | 483（推論のみ・勾配なし） | 32 |
| 時間ヘッド | **1 動画** | **10**（train 動画 10 本） | **420〜1810** |

折りごとの内訳:

| 折り | fine-tune の train フレーム | step/epoch | 時間ヘッドの 1 step のフレーム数（平均） |
|---|---|---|---|
| A | 9,657 | 151 | 420〜1810（966） |
| B | 9,764 | 153 | 420〜1711（976） |
| C | 10,834 | 170 | 420〜1810（1083） |
| D | 10,262 | 161 | 461〜1810（1026） |
| E | 11,182 | 175 | 461〜1118（1118） |

🔴 **config の記録は紛らわしい。** `config.yaml` には `batch_size: 32` と `ft_batch_size: 64` が
両方載るが、**時間ヘッドの run では `batch_size` は使われていない**（`DataLoader` に渡るのは
特徴抽出の経路だけ。`scripts/stage1_ptower_r2.py:200`）。時間ヘッドは
`for _, x, y in data["train"]` で 1 動画ずつ回し `[None]` で batch 次元を 1 にする
（`scripts/stage1_ptower.py:299`）。config だけを読むと batch 32 で学習したように見える。
一周目から続く仕様で本契約が作り込んだものではないが、記録として残す。
