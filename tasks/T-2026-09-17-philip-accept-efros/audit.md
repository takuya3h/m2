...
**戻す `mv` が入れ子を一段作った。** 退避時（分岐 `feat/k1-trace-policy-place`）は
未追跡だった `experiments/**` と契約ディレクトリが、`origin/phase0` 起点の分岐では
**追跡下に既に存在した**ため、`mv src dest` が dest の**中へ**入った。

壊す前に比較した（読み取りのみ）。

| ディレクトリ | 入れ子のみに在る | 両方に在る | 中身が違う |
|---|---|---|---|
| `pd_refin_both_seed42` | 3 件 / 221.8 MB | 2 | **0** |
| `pd_refin_empty_seed42` | 3 件 / 221.8 MB | 2 | **0** |
| `pd_refin_oracle_seed42` | 3 件 / 221.8 MB | 2 | **0** |
| `pd_refin_pred_seed42` | 3 件 / 221.8 MB | 2 | **0** |
| `hts_candidate_acceptance` | **0** | 4 | **0** |
| `tasks/T-2026-09-17-philip-accept-efros` | **0** | 2 | **0** |

修復の順序。**移してから、一致を再確認して、重複だけを消した。**

1. 入れ子のみに在る `checkpoints/` `predictions/` を親へ `mv`（8 回）
2. 再比較して**入れ子のみに在る 0 件・中身が違う 0 件**を確認
3. 重複の入れ子ディレクトリ 6 つだけを `rm -rf`

**失われたデータは無い。**

| 確認 | 実測 |
|---|---|
| 残存する大きさ | `866492` KB（**866 MB**。`checkpoints` 188M × 4、`predictions` 25M × 4） |
| 追跡外である理由 | `.gitignore:24` の `experiments/**/checkpoints/` |
| 追跡下の差分 | `git diff --stat` が**空**（追跡下は無変更） |
| 作業ツリー | `.sync-pause.released` と `docs/sessions/digest/2026-08-25-….md` の 2 件のみ未追跡（開始前と同じ） |

### 抑止の解除

    mv .sync-pause .sync-pause.released

`.sync-pause` は**存在しない**。`.sync-pause.released` のみ。抑止は外れている。

### 最終の状態

| 対象 | 実測 |
|---|---|
| 受け入れ一覧 | **6 件** / 権限 `600` / sha256 `ab3fe1cbbb18534e…` |
| 四ノードとの接続 | `connected=True` **4 件** |
