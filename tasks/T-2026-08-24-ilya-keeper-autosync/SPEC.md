# 常駐処理を配置し、版管理の自動同期だけを動かす（ilya）

**task_id:** `T-2026-08-24-ilya-keeper-autosync`  **kind:** `impl`
**depends_on:** `T-2026-08-22-ilya-node-foundation`
...
**取り込みは手で行う。最初に版管理を最新にすること。**
    ...
    git fetch origin
    git checkout -b feat/ilya-keeper-autosync origin/phase0
    git --no-pager log -1 --format='%h %s'
    mkdir -p tasks/T-2026-08-24-ilya-keeper-autosync
...
**Files:** Create: `tasks/T-2026-08-24-ilya-keeper-autosync/audit.md`
...
**Files:** Create: `tasks/T-2026-08-24-ilya-keeper-autosync/RESULT.md`, `tasks/T-2026-08-24-ilya-keeper-autosync/result.yaml`,
...
- [ ] **Step 2: 送信前に自分で秘匿を検査する**
    ...
    grep -n -i -E "BEGIN [A-Z ]*PRIVATE|api[_-]?key|password|passphrase" \
      tasks/T-2026-08-24-ilya-keeper-autosync/*.md tasks/T-2026-08-24-ilya-keeper-autosync/*.yaml 2>&1
...
- [ ] **Step 4: 変更範囲と未追跡を確かめる**
    ...
    grep -c '' /tmp/ka_ilya.txt; cat /tmp/ka_ilya.txt
...
- [ ] **Step 5: commit し、送出する**
    ...
    git commit -m "feat(sync): deploy keeper and enable git autosync on ilya"
    git --no-pager log -1 --format='%h %s'
    git remote -v
...
**ここで初めて版管理の自動同期が有効になる。**
    ...
    cd ~/slocal2/m2
    mv .sync-pause /tmp/.sync-pause.released.T-2026-08-24-ilya-keeper-autosync 2>/dev/null
