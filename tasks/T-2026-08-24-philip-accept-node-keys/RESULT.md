...
| R | 変更が契約の範囲に限られ、分岐が送出され PR が存在 | §7。commit `e667b4a`/`e11054b`、PR [#142](https://github.com/takuya3h/m2/pull/142) | **満たす** |
| S | 報告が台帳へ返り、抑止が外れている | §7。**台帳への返送は失敗した**（台帳に本契約の行が無い）。抑止は外した | **一部** |
...
### 台帳への返送（失敗した）
    ...
    make task-report TASK=T-2026-08-24-philip-accept-node-keys
    → [task-report] 配布台帳に契約が見つかりません: T-2026-08-24-philip-accept-node-keys
    → exit 2

**SPEC Task 4 Step 6 は「本契約は台帳から取り込んでいるので、返送も通るはずである」と書くが、
配布台帳に本契約の行は存在しない。**

**照合器が壊れているのではない。** 同じ関数で別の契約を引いた結果:

| 契約 | 一度目（`00:15` 頃） | 二度目（`00:26`） |
|---|---|---|
| `T-2026-08-24-philip-accept-node-keys`（本契約） | **無い** | **無い** |
| `T-2026-08-24-philip-syncthing-hub` | **`found=True`** | **`found=True`** |
| `T-2026-08-24-bengio-syncthing-node`（`depends_on`） | 無い | **`found=True`** |

**陽性対照が引けているため、「常に見つからない壊れ方」ではない。**

🔴 **測定の間に台帳の状態が変わった。**
`depends_on` の `T-2026-08-24-bengio-syncthing-node` は一度目の測定では引けなかったが、
`00:25` に `make task-report` を実行すると **`exit 0` で送信に成功**し
（`report_sha256` `7b7d2351…af69`、`report_bytes` `13282`、`replaced_blocks` `0`）、
直後の再測定でも `found=True` を返した。

**一度目の測定は現在の状態を表していない。** 行が後から作られたのか、
台帳の検索が即座に反映されないのかは**測っていない。`UNKNOWN` である。**

**本契約の行が無いことは、時刻を変えて 3 回とも一致した**
（`00:15` の測定、`00:25` の `make task-report`（`exit 2`）、`00:26` の再測定）。

秘匿の検査そのものは通っている。`--dry-run` は `exit 0` を返した。

    {"task_id": "T-2026-08-24-philip-accept-node-keys", "verdict": "partial",
     "n_issuer_defects": 3,
     "report_sha256": "e7ddfdf2c328e6a87830d19981bac4be89e89a4c9a399543a09848441977e558",
     "report_bytes": 21894}

**資格情報を読み込んだうえでの検査に通っており、送信を止めたのは台帳の側である。**

**送信の失敗は契約の完了を妨げない。** 報告そのものは版管理に残している（PR #142）。
**起票者は台帳ではなく PR で読むことになる。**
...
§10 の `result.yaml` に型つきで書く。要点は 4 件。
...
4. **「本契約は台帳から取り込んでいるので、返送も通るはずである」と確かめずに書いた。**
   台帳に本契約の行は無く、指示どおり `make task-report` を実行すると `exit 2` で失敗する
   （時刻を変えて 3 回とも一致）。**`depends_on` の契約は台帳にあるため、
   本契約だけが配られていない。**
