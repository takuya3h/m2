...
## L: task 内の包括的な継続許可後は通常操作の再承認を繰り返さない（2026-08-08）

**症状**: 利用者が「これ以後基本的に認証はいらない」と明示した後も、契約上のGPU承認点で再確認を求めたため、承認要求は不要だと再度修正された。
**原因**: 個別操作の安全確認と、task 全体に対する利用者の包括的な継続許可の優先関係を会話上で十分に反映しなかった。
**ルール**: task の範囲が確定し、利用者が包括的な継続許可を明示した後は、通常の読み取り・外部接続・最小ファイル更新・契約内の実行について再承認を求めない。例外は、削除などの破壊操作、秘密情報への接触、意味のある範囲拡大、未提示の重大な影響が生じる場合だけとする。契約に承認点がある場合も、実行計画を提示したうえで既存の包括許可を適用できるかを先に判断する。

## L: `|| echo` を削除確認へ連結すると直前の失敗を成功表示で隠す（2026-08-12）

**症状**: `git worktree remove ... && git worktree list ... | rg ... || echo removed` で、worktree 削除が未追跡ファイルのため失敗したのに末尾の `echo` が実行され、削除済みのような表示になった。
**ルール**: 削除命令は単独で終了コードを確認する。その後の不在確認も別命令で、実パスと管理側登録をそれぞれ検査する。削除命令と `|| echo removed` を同じ条件式へ入れない。
