...
| 20 | 開始時の未追跡がすべて残っている | 開始 `2` = 版管理外の成果物 `docs/sessions/digest/2026-08-22-95a3a814-….md` + 本契約のディレクトリ。**版管理外の成果物は最後まで未追跡のまま残っている**（commit 後の未追跡 `1` 件はこれ）。本契約のディレクトリは契約の成果物として commit した | PASS |
...
**開始時の未追跡 2 件の内訳は、版管理外の成果物
`docs/sessions/digest/2026-08-22-95a3a814-….md` と、本契約のディレクトリである。**
前者には**触れておらず、commit もしていない**（禁止 1）。後者は契約の成果物なので
commit した。したがって commit 後に残る未追跡は前者の 1 件であり、
**版管理外の成果物は 1 件も失われていない。**
