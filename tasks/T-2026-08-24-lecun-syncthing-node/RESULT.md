...
| V | `m2`: `globalBytes=42010845130` / `globalFiles=5256`、`errors=0` / `pullErrors=0`、`state=syncing`。**T1 20:26:57.693 → T2 20:28:00.749（63.056 秒）で `localBytes` 9285625122 → 15687225893 = 101522468 B/s（≒101.5 MB/s）、`localFiles` 1552 → 2160（9.6 件/s）。残り 26016195954 B。完了は待っていない** |
...
| 同期の速さ | **101.5 MB/s / 9.6 件/s**（実測 63 秒間）。ilya は 75 秒で収束、andrew は約 4 分半。**lecun は不足量が大きく、完了していない** |
...
| `m2` の同期が終わらない | **待たない**（SPEC の指示）。進み方（101.5 MB/s、残り 26016195954 B）を記録して次へ渡す |
