# 常駐処理を配置し、版管理の自動同期だけを動かす（lecun）

**task_id:** `T-2026-08-24-lecun-keeper-autosync`  **kind:** `impl`
**depends_on:** `T-2026-08-22-lecun-node-foundation`
**実行ホスト:** `lecun`  **repo:** `~/slocal/m2`
...
**取り込みは手で行う。最初に版管理を最新にすること。**
    ...
    git fetch origin
    git checkout -b feat/lecun-keeper-autosync origin/phase0
    git --no-pager log -1 --format='%h %s'
    mkdir -p tasks/T-2026-08-24-lecun-keeper-autosync
...
**Files:** Create: `tasks/T-2026-08-24-lecun-keeper-autosync/audit.md`
...
**Files:** Create: `tasks/T-2026-08-24-lecun-keeper-autosync/RESULT.md`, `tasks/T-2026-08-24-lecun-keeper-autosync/result.yaml`,
...
- [ ] **Step 2: 送信前に自分で秘匿を検査する**
    ...
    grep -n -i -E "BEGIN [A-Z ]*PRIVATE|api[_-]?key|password|passphrase" \
      tasks/T-2026-08-24-lecun-keeper-autosync/*.md tasks/T-2026-08-24-lecun-keeper-autosync/*.yaml 2>&1
...
- [ ] **Step 3: 検証を通す**
    ...
    source .venv/bin/activate \
      && make task-validate TASK=T-2026-08-24-lecun-keeper-autosync; echo "validate_exit=$?"
...
- [ ] **Step 4: 変更範囲と未追跡を確かめる**
    ...
    git --no-pager status --porcelain > /tmp/ka_lecun.txt
    grep -c '' /tmp/ka_lecun.txt; cat /tmp/ka_lecun.txt
...
- [ ] **Step 5: commit し、送出する**
    ...
    git commit -m "feat(sync): deploy keeper and enable git autosync on lecun"
    git --no-pager log -1 --format='%h %s'
    git remote -v
...
**ここで初めて版管理の自動同期が有効になる。**
    ...
    cd ~/slocal/m2
    mv .sync-pause /tmp/.sync-pause.released.T-2026-08-24-lecun-keeper-autosync 2>/dev/null
