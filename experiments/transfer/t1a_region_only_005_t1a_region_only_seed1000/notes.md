...
凍結 Relation-DETR seed42 の object-query 埋め込み（クラス別 256-d, score 加重）を GAP(2048) に 連結せず単独で入力 → 素 causal TeCNO。
勾配は交差させず凍結 region token を渡す。online/causal（未来不使用）。

## 結果（best @epoch 32）
- accuracy=0.9459 / macro_f1=0.7993
- edit=36.30 / seg_f1@10/25/50=0.45/0.44/0.40

## 構成
- seed=1000 epochs=50 lr=0.0005 in_dim=3840 stages=2 layers=8 f_maps=64
