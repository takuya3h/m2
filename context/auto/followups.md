...
- 台帳への返送が失敗した。make task-report が「配布台帳に契約が見つかりません」で exit 2 を返す。 台帳に本契約の行が無い（時刻を変えて 3 回とも一致）。照合器は働いており T-2026-08-24-philip-syncthing-hub と T-2026-08-24-bengio-syncthing-node は found=True を返す。 後者は報告を exit 0 で送信できた。起票者は台帳ではなく PR #142 で報告を読むこと。 台帳へ載せる必要があるなら、行を作ってから make task-report を再実行すれば通る。
...
### T-2026-08-25-issuer-refs-to-repo

- conventions.md の変更履歴は naming 節の本文の内側にあり、追記すると naming の解決結果が変わる。 変更履歴の直前に終端用のアンカー（例 <a id="changelog"></a>）を置けば、以後の追記が 既存の節を汚さなくなる。本契約は追加のみのため触っていない。
- CLAUDE.md:74 と docs/notion_integration.md:30 は NOTION_DB_ID を .env の変数として挙げるが、 load_env.sh 後の実測で unset だった。版管理側の文書が古い。本契約の範囲外のため直していない。
- P9 spec_lint の separated_source@SPEC.md:50 は誤検出である。source ... && source ... && make が 行継続の \ で 2 行に分かれているものを、読み込みが単独の命令で終わったと判定している。 check_agent_docs.check_text 側で行継続を連結してから判定すべきである。
- forbidden-check は「追加のみ」を判別しない。conventions.md への追記は本契約のように 正当な場合があるが、道具は変更の有無だけを見て違反を返す。判定の根拠は人が書くしかない。
...
### T-2026-08-26-denoise-falsification

- 本番入口 scripts/train_b2a.py に雑音除去の腕を足す契約が要る。実装は docs/analysis_scripts/hmm_presence_filter.py の forward_filter を流用でき、出力は生信号と同形 (T,15) の [0,1] である。信号を読んだ直後に通す形が、既存の --tool-source と同じ位置で副作用が小さい
- 代替として、デノイズ済みの信号キャッシュを作って RELDETR_FROZEN_TAG で差し替える方法もあるが、この環境変数は GAP のディレクトリも同時に切り替えるため GAP 側の複製が要る。腕を足す方が小さい
- 本契約の分母（frozen_tecno_phase_baseline@val~relation_detr_seed42）は 8 コミットにわたり不変である。accuracy_mean 0.8973014948553679 / edit_score_mean 39.91661295170067 / sstd 2.621371700613755 / n_seeds 3。次の契約はこれをそのまま使える
- 投影と集約（make taskindex / make inbox）は本契約では回していない。四台の PR が全て統合された後に、一台で一度だけ回すこと
- 起票の際に meta.created_from を最新の runindex から取ること。本契約は 2 週間古い索引の状態から起票されており、検証が WARN を出した

### T-2026-08-26-det2phase-segmentation-lovo

- 代理モデルで見えた分節側の改善は、本番の時系列モデルの一つ抜き検証でも成立した。主終点 edit は Δ +10.00606 で符号 11/15、確定則 R1 が検出した。seg-F1@50 は Δ +0.12316 で符号 14/15、R1 が検出した。撤回した主路に代わる骨格として報告できる材料になる。
- ただし主終点の判定は脆い。R1 の閾 2.0 に対し統計量は 2.200 で、動画 01 または 12 を除くと 1.988 / 1.990 へ落ちて反転する。seg-F1@50 は統計量 2.838 でどの 1 組を除いても検出のままである。骨格として報告するなら seg-F1@50 の方が支えとして強い。
- 本番の結果は、構造的に一致する代理の比較 C04 より強い。C04 は代理側で R1 の判定を失っていた（edit 1.960 / seg-F1@50 1.425）が、本番では 2.200 / 2.838 で検出した。代理で弱かったものが本番で強くなった例であり、代理から本番への外挿は向きが一定でない。
- 契約が引く C01 と、契約が作れと指示した腕に当たる C04 は別の比較である。C01 は GAP を丸ごと落として術具だけにする比較で Δ edit +32.20、符号 15/15。C04 は GAP に術具を足す比較で Δ edit +4.76、符号 12/15。次の起票では、どの比較の結論を検証するのかを腕の定義と一致させること。
- R2 を本番のデータへ当てるには反復 LOVO が要る。24 反復 x 12 fold x 2 腕 = 576 run と、動画母集団を絞る配線である。1 本 40 秒として約 6.4 時間。別契約として起票すれば実行可能な規模である。
- 最良 epoch を抜いた動画の正解で選んでいる。主終点が分節側であるのに選択は分類側の指標で行われており、禁止 9 の趣旨と噛み合わない。学習側から検証用の動画をさらに 1 本抜いて epoch を選ぶ設計にすれば解消するが、学習の手続きを変えることになるため起票者の判断事項である。
- 禁止 5 のため投影と集約を再生成していない。全 PR 統合後に一台で make taskindex と make inbox を回すこと。make runindex も回していないため本契約の 30 run は索引に現れていない。

### T-2026-08-26-error-shape-selectivity

- .stignore:52 の !experiments/**/*.py は experiments 配下の .py を同期対象にする。契約の実行中に 他ホストの状態で削除されうる（本契約と lovo_decision_rule の計 4 件が実測で消えた）。 実装の置き場所を同期の外にするか、除外規則を見直すかの判断が要る。
- .sync-pause は scripts/sync/m2-sync.sh:40 の git 操作しか止めない。syncthing の実ファイル同期は 止まらない。手順書と SPEC はどちらも「抑止の目印を置く」としか書いておらず、適用範囲が明示されていない。
- forbidden-check の起点 origin/phase0 は契約の実行中に動く。BASE を task-start 時点の commit へ 固定できるようにするか、起点のずれを出力に含めるかの判断が要る。
- 既存の記述「評価側だけ汚すと選択性が 3.9 倍 → 7.0 倍に鋭くなる」（§3.10(c)）は p=0.05 の一点でのみ 成立し、p ≧ 0.10 で逆転する。訂正の要否は decisions_required で「契約の外」と回答されたため 本契約では触れていない。
- proxy_lovo_noise_testonly.py は build(...,7) と種を直書きしており、種 1 本の結果しか出さない （:31-32）。記録の 10.5 と本掃引の 10.04 の差はこれで説明がつく。原典を 3 本へ揃えるかの判断が要る。

### T-2026-08-26-lovo-decision-rule

- 四台の PR がすべて統合された後、一台で一度だけ make taskindex と make inbox を回すこと。本契約は契約 第 5 節禁止 4 により再生成を行っていない
- 採用する判定則を一つに決めること。本契約は R0・R1・R2 の結果を並べたところまでで、正本の指定は起票者と利用者の判断事項である。決めた則は、同時に走っている別ホストの契約が出す効果量と符号の個数へ後から当てられる
- 既存の報告 docs/research_review_and_next_plan_2026-08-22.md のうち訂正の対象になりうる箇所を REPORT.md 第 11 節に一覧にした。訂正するかどうかは起票者と利用者の判断事項である
- 禁止領域の検査 make forbidden-check が、契約の outputs.destination を除外しないため必ず失敗する。destination を除外する仕組みを入れるか、分析成果物の置き場を experiments/ の外へ移すかの判断が要る
- 同期が experiments/**/*.py を対象にしており、実行中の作業ファイルを削除する。契約の抑止 .sync-pause は自動統合しか止めない。ファイル同期も止める目印を用意するか、作業を experiments/ の外で行う規約が要る
- R2 の共有成分は学習側 11 動画で測った。本番相当の 14 動画で測り直すと補正の強さが変わりうる
...
### T-2026-09-18-stage1-detector-towers

- D*-ImageNet は 12 epoch で打ち切られている可能性が高い。最終 epoch が最良で、最後の 1 epoch の伸びが D*-COCO の 5.2 倍（+0.0067 対 +0.0004）である。epoch を増やしたときの到達点は測っていない。H3 の材料として両塔を使う前に、打ち切りの影響を見積もるか epoch を揃え直すかの判断が要る
- 予測 2（ImageNet は 5〜15 mAP 低い）が外れ、実測は 1.66〜7.33 mAP と予測より差が小さかった。両塔とも打ち切られている可能性があるが切り分けていない。証拠地図 A の SurgMAE Table 7 からの外挿（距離 2〜3）の当たり方として記録する価値がある
- 折り C・D・E で test mAP が val mAP を上回る（折り E は val 0.5272 に対し test 0.5841）。折りごとの難しさが val と test で逆向きに出ている。折り表の設計に起因するなら Tier 1 の読み方に影響する
- contract.allow_write の宣言漏れが 2 契約続けて起きた。experiments/ へ書く run を伴う契約では起票の雛形に入れるか、Task C 相当の run がある契約で自動的に補うかの判断が要る
- 完了判定の空振り確認に「収穫前後の行数の差 = 新実験数」を置くのはこの repo では誤りである。収穫器は既存行も更新し同期で届いた退避物も拾う。雛形を直すかの判断が要る
- preflight の P3 deterministic_flags は宣言しても SKIP される（判定基準が未確定・backlog B-20）。宣言しても検査されない項目があることを起票側が知っている必要がある
...
### T-2026-09-19-p13-skip-and-enum

- 既存の result.yaml 1 件（T-2026-08-22-philip-hub-foundation）が schema を通らない。旧様式で meta: の下に task_id と verdict: PARTIAL を置く形で書かれており、最上位の task_id と status を欠く。変更前から不通過で本契約とは無関係のため契約 §6 に従い触っていない。旧様式を通す判断か、当該報告を現行様式へ移す判断かを起票者が決めること
...
- depends_on の T-2026-08-24-bengio-syncthing-node が一度目の測定で引けず二度目で引けた理由。行が後から作られたのか台帳の検索が即座に反映されないのか、どちらかを測っていない
...
- 一般の節の本文抽出の壊れ方。切り出しを機械化した実装は preflight_task.py:109-126 の frozen_source 専用の 1 か所だけで、汎用の抽出器が無い。抽出は実行者の手続きであるため、 抽出側が静かに空を返す壊れ方があるかは測れていない。
- .env の変数がちょうど 5 件かどうか。照合したのは配られた 5 名 + NOTION_DB_ID の計 6 名だけで、 一覧に無い名前が他に在るかは確かめていない。.env 本体の読み取りは実行基盤が拒否した。
- 起票者の手順書が 671 行から 341 行へ縮む見込み。手順書は版管理の外にあり、このホストから測れない。
...
- 学習一本の所要時間。学習を走らせていないため測っていない
- 残り時間で入る対の組数。所要時間が無いため計算できない
- 見える効果の大きさの下限。対測定の本数が 0 のため求まらない
- 効果量および改善側へ倒れた対の個数。対測定を行っていないため観測が 1 件も無い。効果が見えなかったのではなく、測っていない
- 本番側へ雑音除去の腕を足すのに要する時間。実測できないため推測を数値で書かない
- 試験の件数は 0 / 0 / 0 と記録したが、これは本契約が src や scripts のコードを一切変更していないためであり、試験一式を走らせた結果ではない

### T-2026-08-26-det2phase-segmentation-lovo

- R2 の判定。反復 LOVO を走らせていないため UNKNOWN。効果量と符号の個数は則に依存しない形で残してある。
- 種は 42 の 1 種のみである。分け方ごとの Δ に種の揺らぎがどれだけ入るかは測っていない。
- 最良 epoch を抜いた動画の正解で選ぶことによる楽観の大きさ。両腕に等しくかかることは構造から言えるが、大きさは測っていない。
- 本契約の腕（gap+pres - gap）を、契約が引く C01 の腕（pres - gap）に合わせた場合の本番の値。--drop-gap で作れるが、契約 §2.1 が他の入力を落とすことを禁じているため走らせていない。
- runindex への反映。make runindex を回していないため index.csv は本契約の 30 run を含まない。config.yaml への task_id の刻印は 30 本すべてで確認したが索引そのものは未更新。
- fold 01 の失敗した試行（seq 0）の開始時刻。記録していなかったため UNKNOWN。所要 2 秒と RC=2 は実測である。

### T-2026-08-26-error-shape-selectivity

- 最も選択性が大きい角（p=0.01, L=1・評価側のみ）の倍率。分母 Δacc の効果量が 1.87 と格子で最小で、 種ごとの選択性が 57.07/34.74/35.56 と幅を持つ。3 本とも 34 を超えるため「大きい」ことは支持されるが、 倍率の値は定まらない。
- 本番の構造で同じ選択性が現れるか。本掃引はプロキシ（線形分類器 + HMM の LOVO）であり、 本契約の範囲外である。
- 高い p での L 軸に沿った比較の解釈。実測誤り率が名目の 0.71〜0.83 まで落ちるため、 L を変えた比較は実際の誤り量を揃えた比較になっていない。p=0.40 の行はとくに影響が大きい。

### T-2026-08-26-lovo-decision-rule

- 本番 TeCNO の上での再判定。本契約はプロキシ（多項ロジスティック回帰 + 因果 phase HMM）の上でのみ測った
- 学習側が 14 動画のときの共有成分の大きさ。R2 は部分集合 12 本（学習側 11 動画）で測っており、同じとは限らない
- 受容野と凍結源の 6 台本を 24 反復にしたときの R2。費用の都合で 12 反復に留めた
- 開始前の試験の件数を先に測らなかった。before_failed の 5 は完了後の実測値であり、本契約の差分が experiments/analysis/lovo_decision_rule/ の外に 0 件・src/ と tests/ に 0 件であることを実測して同値と判断した。直接の測定ではない
...
- D*-ImageNet を 12 epoch より延ばしたときの到達点。最終 epoch が最良で末尾の傾きが大きく打ち切りが疑われるが測っていない
- 予測 2 の差が小さかった理由。両塔とも打ち切られている可能性があるが切り分けていない
- 折り C・D・E で test mAP が val mAP を上回る理由。原因は調べていない
- Stage 1 の正式な工程塔の処方。ilya の契約 T-2026-09-18-stage1-phase-tower が確定中
- TF32 で回した場合の所要時間と主指標。凍結源が fp16 のため TF32 では回していない
