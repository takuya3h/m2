...
判断・解釈・評価が必要な研究方針は**外部の面**（Claude アプリが読む運用正本・
現在地と現行計画・マスター）の役目である。**CLI はそれらを読まない。**
CLI が Notion に触れるのは配布台帳だけで、手順は `docs/notion_integration.md` にある。
ID の登録簿は `configs/notion.yaml`（非秘密）。
契約ごとの判断は `tasks/inbox.d/`、教訓は `tasks/lessons.md` に置く。
