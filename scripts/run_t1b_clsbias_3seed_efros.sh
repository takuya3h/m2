...
for S in "${SEEDS[@]}"; do
  echo ""
  echo "############### seed$S ###############"
  MEAS=/tmp/t1b_clsbias_measure_seed${S}
  echo "[measure seed$S] init mAP を実測（--epochs 0, GPU$GPU_INJ）..."
  run_one "$S" "$GPU_INJ" "$MEAS" "logs/t1b_clsbias_measure_seed${S}.log" --epochs 0
...
PY
  ...
  INJ=/tmp/t1b_clsbias_seed${S}
  CTRL=/tmp/t1b_clsbias_zeroctx_seed${S}
