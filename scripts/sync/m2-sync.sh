...
# --- 契約実行中の一時停止 ---
# 目印のファイルがある間、このスクリプトは分岐へ一切書き込まない。
# TASK 契約は「統合しない / 自動統合を有効化しない」を禁止事項に置くが、本スクリプトは
# 実行者の操作なしに作業分岐へ auto-merge する。実行者の努力では守れないため、
# 契約の実行中だけ機械的に止める。
#
# 止めるのは **書き込みだけ** である。keeper は従来どおり 30 分周期で回り続け、
# 目印を消せば次のループから元に戻る（常駐処理そのものは停止・削除しない）。
#
