...
SRV="${SERVERNAME:-$(hostname)}"   # コンテナのhostnameは実サーバー名と一致しないことがある
mkdir -p "$(dirname "$LOG")"
cd "$M2DIR" || exit 1

git fetch -q origin || {
  echo "$(date '+%F %T') [$SRV] fetch失敗(PAT期限切れ?)" >> "$LOG"
  exit 1
}
...
if [ "$BR" = "$MAIN" ]; then
  ...
  git merge --ff-only "origin/$MAIN" -q ||
    echo "$(date '+%F %T') [$SRV] ${MAIN}更新失敗(未コミット変更と衝突)" >> "$LOG"
else
  git fetch -q origin "$MAIN:$MAIN" ||
    echo "$(date '+%F %T') [$SRV] ローカル${MAIN}が分岐(${MAIN}に直接コミットした?)" >> "$LOG"
fi
