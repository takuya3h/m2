...
| B-30 | **`m2-sync.sh` のアラート文言「phase0更新失敗（未コミット変更と衝突）」が不正確** | 実際の失敗原因は「未追跡ファイルの上書き拒否」であり、追跡ファイルの変更ではない。2026-08-04 に未整備 5 ノード（`Hinton` / `adam` / `DL-Station` / `he` / `ian`）を調査したところ、5 台とも **追跡ファイルの変更 0 件**で、`git status` に出ていたのはすべて未追跡ファイル（Syncthing 由来の `logs/eval_meta_val.json` と `logs/val_metrics_by_epoch.json`）だった。件数は `Hinton` / `adam` / `DL-Station` / `he` が各 58 件、`ian` が 4 件。git は未追跡ファイルの上書きを**バイト単位で同一でも拒否する**ため `git merge --ff-only origin/phase0` が失敗し続けていた（アラート 28〜32 件 / 台）。`ian` だけ成功していたのは未追跡 4 件が phase0 に存在しなかったため。文言が誤っていたため、調査するまで「未コミットの成果が眠っている」と誤読された（実際は 5 台とも未追跡 `metrics.json` 0 件で、固有の実験成果はゼロ）。 | git のエラー出力を判別して文言を分ける。`untracked working tree files would be overwritten by merge` → 「未追跡ファイルと衝突（Syncthing 由来の可能性）」、`Your local changes to the following files would be overwritten` → 「未コミット変更と衝突」。なお `m2-sync.sh` は `~/bin/keeper.sh` から呼ばれる**外部管理スクリプト**であり、リポジトリの `scripts/sync/m2-sync.sh` を変更しても各ホストには自動配布されない。配布方法の確認が先に必要。 |
