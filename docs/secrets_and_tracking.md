...
鍵を更新しても、**各ホストに古い平文 `.env` が残っていればそのホストだけ古い資格情報で
走り続ける。** 暗号文を更新して push しただけでは伝播しない。更新した本人以外の全ホストで
次を行うこと。

```bash
cd /home/ubuntu/slocal2/m2
git pull                      # 1) 更新された .env.gpg を受け取る
rm -f .env                    # 2) 古い平文を消す（これを飛ばすと 3 が警告で止まる）
source scripts/load_env.sh    # 3) 暗号文から読み直す
```

`rm` を飛ばすと `load_env.sh` が食い違いを警告する。**警告は放置しない。**
どちらを正とするかは前節のとおり選ぶ。警告に気づかず走らせると、そのホストだけが
古い資格情報を使い続け、失敗が「そのホストでだけ再現する」形で現れる。

### ホスト固有の値を共有の設定へ入れない

**共有の設定は全台へ同じ内容が配られる。ホストごとに違う値は持てない。**
ホスト固有の値を入れると、写した全ホストが同じ値を名乗る。

実際に起きた事故: 論理名 `SERVERNAME` が共有の設定に入っていたため、**全台が同じ論理名を
名乗り**、どの実験がどのホストで走ったのか区別できなくなった。原因の特定に時間を要した。
共有の暗号設定からは `c61a673` で除去済みである。

| 種類 | 置き場所 |
|---|---|
| 全台で同じ値（API 鍵・プロジェクト名など） | 共有の設定（`.env` → `.env.gpg`） |
| **ホストごとに違う値（論理名など）** | **各ホストの起動時設定**（`scripts/sync/setup_host_servername.sh` が `~/.zshenv` 等へ注入する） |

**規約: 共有される配布物には、ホストごとに違う値を書かない。** 対象は共有の設定
（`.env` → `.env.gpg`）だけではない。**雛形 `.env.example` も共有される配布物である。**
雛形は全台へ同じ内容が配られるため、そこに具体名を書くと、写した全ホストが同じ値を名乗る。

雛形に残っていた `SERVERNAME=bengio` は 2026-08-15 に除去した。`c61a673` は暗号文だけを
直し、**雛形を直していなかった**ため、新しいホストを立てるたびに再発する導線が残っていた。
実測では、ホストの正しい論理名を先に置いた状態でこの雛形を読み込むと、その名前が雛形の値へ
**上書き**された（bash と zsh の双方で再現）。空欄を埋めるだけの無害な挙動ではない。

コメント中の例示にも具体名を書かない。読んだ人がそのまま写す。

**この規約に機械的な検査は無い**（`B-39` として起票済み）。雛形や共有設定へ値を足すときは、
「全台で同じ値であるべきか」を人が判断すること。同じであるべきなら共有へ、違うなら
各ホストの起動時設定へ置く。
