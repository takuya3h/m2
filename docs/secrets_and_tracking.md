...
🔴 **`.env.example` は 2026-08-10 時点で `SERVERNAME=bengio` を含んだままである**
（17 行目）。`c61a673` は暗号文だけを直し、テンプレートを直していない。
**このテンプレートをそのまま写すと同じ事故が再発する。** 写すときは `SERVERNAME` の行を
落とすこと。テンプレート自体の扱いは未決（`tasks/inbox.d/` に起票済み）。
