...
## 2026-09-17 — 凍結検出塔の出力キャッシュは P→D の界面 run を速くしない（識別実験）

**仮説**: P→D 側の W1 界面 run が 1 本約 4 時間かかる原因は、凍結した検出塔へ毎 epoch
画像を通し直していることである。塔の出力を一度保存して界面だけを学習すれば、同じ結果の
まま大幅に速くなる。

**実験**: 基準 run `pd_refin_empty_seed42`（T-2026-08-29・lecun）と同じ処方
（`--inject film --trainable film --phase-source real --zero-ctx --seed 42 --epochs 6`、
batch 2、折り A）を efros（A6000）で再現し、(a) 1 step を要素へ分解、(b) 凍結 backbone の
出力をキャッシュして同一バッチ集合で二経路を比較、(c) 決定性と陰性対照、(d) W2 相当
（`--trainable all`）でも同じ比較、を実測した。読み出しは頁キャッシュを落として測っている。

**結果**:

| 量 | 実測 |
|---|---:|
| 1 step | 0.4937 s（2.03 it/s）。1 epoch 41.8 分、6 epoch 4.18 h（記録の「約 4 時間」と整合） |
| 凍結塔の順伝播 | step の **4.63〜7.68%**（差分法・4 条件） |
| 上限倍率 | **1.049〜1.083×** |
| キャッシュ経路の実測 | **0.558〜0.661×（1.5〜1.8 倍遅い）** |
| 損益分岐に要る読み速度 | 2.28〜4.58 GB/s。実測 0.367 GB/s（経路内）／0.472 GB/s（`dd`・direct） |
| キャッシュの大きさ | 1 枚 45,783,450 bytes（学習・増強後）。1 epoch 440.3 GB、6 epoch 2.64 TB |
| 決定性 | 二度生成で `aggregate_sha256` 一致。塔を seed123 へ変えると変化（陰性対照） |
| W2/W1 の 1 step 比 | 1.205（学習対象 266,880 → 25,505,568 param） |

**解釈**: 仮説は**否定された**。時間の約 85% は FiLM より後段の transformer の順伝播と
逆伝播であり、入力が界面の重みに依存するため原理的にキャッシュできない。さらに塔の出力
（1 枚 45.8 MB）は元画像より桁違いに大きく、読み出しが再計算より高くつくため、キャッシュは
速くならず遅くなる。学習の前処理が毎 epoch 乱択であることも重なり、結果を揃えたまま
キャッシュするには epoch ごとに別物（6 epoch で 2.64 TB）が要る。

**次**: 実装水準の手当てでは 4 時間は縮まない。判断は設計水準へ移る。時間を縮めるなら
削るのは腕ではなく 1 run の中身（epoch 数・解像度・query 数・encoder 層数）であり、
いずれも比較の土台を動かすため**全腕で同一に動かす**ことが条件になる。
Tier 1 の日数は変わらず、縮退なしで 153.1〜208.7 日のままである。

証跡: `docs/stage0/C1_frozen_feature_cache_timing.md` /
`tasks/T-2026-09-17-frozen-feature-cache-timing/{RESULT.md,audit.md,result.yaml,evidence/}`。
`experiments/` `data/raw` `data/external` `runindex/` `context/conventions.md` には変更を加えていない。

---

## 2026-09-17 — 数値精度は効き、結果は変わらない。計算グラフの最適化は使えない

**仮説**: 単精度で回している検出側の界面 run を、数値精度を下げる設定と計算グラフの最適化で
短くできる。短くできるなら主指標は seed 間の散らばりの内側に収まる。

**実験**: 基準 `pd_refin_empty_seed42` と同じ処方（`--inject film --trainable film
--phase-source real --zero-ctx --seed 42 --epochs 6`、batch 2、折り A）について、
(a) 条件ごとの 1 step 時間を同一バッチ列・同一ホストで測り、(b) 最良条件で折り A を
6 epoch 回して主指標を基準と比べた。暖機を捨て、装置の同期を取ってから計時している。
同期を取らない値も併記した。

**結果（条件ごとの 1 step、efros・A6000）**:

| 条件 | W1 | 倍率 | W2 | 倍率 | 記憶の山（W1） |
|---|---:|---:|---:|---:|---:|
| fp32（現行） | 0.4611 s | 1.000× | 0.5652 s | 1.000× | 14,590,299,648 B |
| **tf32** | **0.3902 s** | **1.182×** | **0.4396 s** | **1.286×** | 14,589,924,864 B |
| bf16 | 0.3927 s | 1.174× | 0.4503 s | 1.255× | 12,726,704,640 B |
| fp16 | 0.3945 s | 1.169× | 0.4613 s | 1.225× | 12,726,155,776 B |
| compile 系 3 条件 | 完走しない | — | — | — | — |

短い測定（40 step）と長い測定（200 step）で倍率が一致する（tf32 1.195/1.182 等）。
同期を取らないと条件ごとに 1.2〜12.3% 速く見え、**倍率そのものが歪む**。

**結果（主指標、tf32 で 6 epoch）**: σ = 0.004540（`t1b_filmonly` の 3 seed、pstd）。

| 指標 | 基準 fp32 | tf32 | 差 | abs(差)/σ |
|---|---:|---:|---:|---:|
| init | 0.7302938995 | 0.7302930990 | −0.0000008004 | 0.0002 |
| best | 0.7335755045（ep2） | 0.7337119334（ep2） | +0.0001364288 | **0.030** |
| final（ep5） | 0.7313710102 | 0.7313652919 | −0.0000057183 | 0.0013 |

全 epoch で最大 0.138σ。最良 epoch も一致。損失 150 点で非有限 0 件・中央の 3 倍超 0 件。
run 内の実測倍率は **1.171×** で、Phase B の予測 1.182× と 1% 以内で一致した。

**解釈**: 前契約 `C1` が示したとおり時間の約 85% は transformer の順伝播と逆伝播であり、
そこは**演算そのもの**である。だから演算の精度を落とす手当ては効き（1.18〜1.29×）、
データ供給やキャッシュの手当ては効かなかった（上限 1.08×・実測 0.65×）。**効く場所は一貫している。**
`torch.compile` は `set_criterion.py:199` の `F.one_hot` が記号のままの形を扱えず完走しない。

**次**: Tier 1 の日数は 12h/日・Stage 1 + Tier 1 で 159.9〜218.2 日 → 135.3〜184.7 日。
縮退段 3 の IPCAI long abstract が「読みにより分かれる」から**「収まる」**へ変わる。
**採用の可否は利用者が決める**（本契約は採用しない）。速さでは tf32 と bf16 に差が無く、
記憶領域では bf16 が優り、変更の小ささでは tf32 が優る。採用すると既存の単精度 run とは
比較の土台が変わり、分母の再構成が要る。

証跡: `docs/stage0/C2_amp_compile_timing.md` /
`tasks/T-2026-09-17-amp-compile-timing/{RESULT.md,audit.md,result.yaml}` /
`experiments/transfer/pd_refin_empty_seed42_tf32/`。
`data/raw` `data/external` `runindex/` `context/conventions.md` には変更を加えていない。

---

## 2026-09-18 Stage 1 工程塔: 特徴抽出開始（ilya）

- 仮説: ImageNet V1の凍結R50から決定的なGAP特徴を得て、検出backbone由来のS4から独立したP塔を比較できる。
- 実験: `T-2026-09-18-stage1-phase-tower`。追加6動画の画像が未発見のため利用者承認のP15を対象に全15,437フレームを二度抽出。W&B識別子の保存を確認。
- 結果: 初回はHydraのstruct設定へ証拠キーを追加してConfigKeyErrorとなり、特徴計算前に停止。コピーへの追加へ修正し再起動。関連テスト14件通過、ruff通過。二度の照合結果は実行中。
- 解釈: 初回失敗は設定保存経路の問題であり、モデル性能の測定ではない。
- 次: 特徴の二度一致・重み変更対照を確認後、折りA seed42の線形対照と候補Aの停止条件を測定する。

## 2026-09-18 Stage 1 工程塔: 三候補トーナメントと P*-15 の確定（ilya）

- 仮説: 凍結 ImageNet-1K R50 の空間特徴の上では、causal 2 段 TCN（候補 A）が
  Transformer 集約（候補 B）と平滑化損失（候補 C）より高い折り内 val macro Jaccard を出す。
  根拠は同ドメインの否定例（EgoSurgery-Phase で Trans-SVNet 23.1 < TeCNO 27.3）。
- 実験: `T-2026-09-18-stage1-phase-tower`。10 構成（A 2・B 4・C 4）× 5 折り
  （折り A は seed 42/123/456、B〜E は seed 42）= **70 run**。加えて折り A の単フレーム線形対照 1 run。
  追加 6 動画の画像が本ホストに無いため **P\*-15 のみ**（P\*-21 は未実施）。
  評価は online-causal・Jaccard strict。選定は val だけで行い、test は確定塔の 5 折りに 1 回ずつ。
- 結果: 5 折り平均 val macro Jaccard の上位は C/L8/w0.15 が 0.33280、C/L8/w0.30 が 0.32891、
  B/L8/h30 が 0.32300。**候補 A の最良は 0.30747 で 10 構成中 7 位**。
  最良と次点が seed 間 SD 以内・所要時間 ±20% 以内で並び、修正条項の同点規則でも決着しなかったため、
  事前登録 §6.3 に従い諮って **C・8 層・平滑化 0.30・履歴 30** を確定した。
  確定塔の 5 折り test は macro Jaccard 0.22472 / frame accuracy 0.54535。
  折り A の val 0.36689 は Stage 0 の S4（検出 backbone 由来、val 0.64474）を大きく下回る。
  単フレーム線形対照 0.22495 に対し候補 A の折り A 6 run はすべて上回った（最小の差 0.03418）。
- 解釈: **事前登録の予測 1 と 2 が外れた。** 凍結 ImageNet 特徴の上では時間ヘッドの序列が
  S4 の検出 backbone 特徴の上と同じにならず、根拠にした同ドメインの否定例は本設定で再現しない。
  平滑化損失（候補 C）が効いたことは、この特徴が時間的に不安定で、出力側の平滑化が効く余地が
  大きいことを示唆する。S4 との 0.2779 の差は特徴の出所と時間ヘッドの両方が異なるため分解できない。
  折り B・C が全構成で低く（B 0.19-0.24、C 0.23-0.29）、折りによる難易度差が 5 折り平均の解釈に効く。
- 次: 追加 6 動画の画像を使えるようにする別契約を立て、P\*-21 を同じ recipe で学習する
  （Stage 2 の主分母はこちら）。検出塔 D\* は別契約。折りごとの難易度差の要因も Stage 2 の前に調べる。

## 2026-09-18 Stage 1 工程塔の二周目: backbone の fine-tune と時間ヘッドの再トーナメント（ilya）

- 仮説: 検出塔 D\* と同じく backbone を手術データ（工程ラベル）で fine-tune すれば、
  凍結 ImageNet 特徴の一周目（val 0.329 / test 0.225）を大きく上回り、Stage 0 の S4
  （検出 backbone 由来、折り A val 0.645）との差が半分以下に縮む。時間ヘッドの最良は変わりうる。
- 実験: `T-2026-09-19-stage1-phase-tower-r2`。**stem（conv1・bn1）のみ凍結**して layer1〜4 と fc を
  折りの train 動画の工程ラベルで学習（検出塔の `freeze_indices=(0,)` と対称）。学習率 2 水準
  （1e-4 と その 1/3 の 3.3333e-5）× 5 折り（折り A は seed 42/123/456）= **14 fine-tune**。
  各 backbone から一周目と同じ前処理で C5 GAP を抽出（**14 種**）し、一周目と同じ時間ヘッド 10 構成を
  その上に載せて **140 run**。選定は val だけ、test は確定塔の 5 折りに 1 回ずつ。
  追加 6 動画の画像が本ホストに無いため **P\*-15 のみ**（P\*-21 は未実施）。
- 結果: fine-tune は 14/14 が val frame accuracy で一周目の単フレーム線形対照 0.48383 を上回った
  （+0.043〜+0.236）。確定 recipe は **候補 C・6 層・平滑化 0.15・履歴 30・backbone 学習率 1e-4**。
  5 折り平均 val macro Jaccard **0.38710**（一周目の確定塔 0.32891 から +0.05819）、
  test **0.25618**（一周目 0.22472 から +0.03146）。折り A val は 0.36689 → **0.46789** で、
  S4 との差は 0.27785 → **0.17685**（36.4% 縮小）。
  🔴 **学習率で完全に分かれた。** 高い方 1e-4 の 10 構成がすべて上位（0.36173〜0.38710）、
  低い方 3.3333e-5 の 10 構成がすべて下位（0.27942〜0.32649）で、
  **低い学習率では一周目の凍結特徴とほぼ同等かむしろ悪い**。
- 解釈: **事前登録の 5 予測のうち当たりは 1 件だけだった**（val が 5 点以上上がる、のみ）。
  fine-tune は効くが、効き方は予想と違う。(1) S4 との差は縮んだが半分以下には届かない
  ——特徴の出所の差は fine-tune だけでは埋まらない。(2) 低い学習率が安定するという一般則は
  この設定では成り立たず、**上位を高い方が独占した**。凍結 stem 以外を 12 epoch で動かすには
  1e-4 でも足りない側にあるとみられる。(3) 最良は一周目と同じ候補 C だが掃引点は
  8 層・0.30 から 6 層・0.15 へ動いた——特徴が強くなると受容野も平滑化も小さい方が良くなる。
  (4) **val と test の乖離は 0.104 から 0.131 へ広がった**。fine-tune が折りの train 動画へ
  適合した結果とみられるが、本契約は要因を分解していない。
- 次: 追加 6 動画の画像を使えるようにする別契約を立て、P\*-21 を同じ recipe で学習する
  （Stage 2 の主分母はこちら）。学習率は 1e-4 より高い側を見る価値がある。
  最良 epoch が上限 12 に張り付いた run が 2 件あり、上限が結果を切っている可能性も残る。
  S4 に届かない場合は backbone の種類（ViT、中立 SSL）が次の軸になる（SPEC §8 の申し送り）。

## 2026-09-21 — Stage 1: 検出塔 D\*-COCO と D\*-ImageNet を 5 折りで確定した

**仮説（事前登録 `fa9cc546`、最初の学習の 15 分前に commit）**: 凍結源の処方で検出塔を 5 折りに
広げると、(1) 折り A seed42 の COCO 塔は凍結源と 1 mAP 以内、(2) ImageNet 塔は全折りで 5〜15 mAP 低い、
(3) 折り間 SD > 折り A の seed 間 SD、(4) ImageNet は最良 epoch が後ろに寄る。

**実験**: 凍結源を生んだ処方を run の証跡から特定し（UNKNOWN 0 件）、そのまま 14 run
（2 塔 × 5 折り、折り A は 3 seed）を efros の A6000 2 枚で回した。

🔴 **契約は「全 run を TF32」と定めていたが、凍結源の実測は fp16 だった**
（`command.sh` の `--mixed-precision fp16`）。前契約 `C2` の TF32 の測定は fp32 の界面 run が
基準で、検出塔のフル学習には当てはまらない。利用者の判断で **fp16 に揃え、処方の差分を 0 項目**にした。

**結果**:

| 塔 | 折り A（3 seed 平均） | 折り B〜E の平均 | 折り間 pstd | seed 間 pstd |
|---|--:|--:|--:|--:|
| D\*-COCO | **0.7235** | 0.4846 | 0.032249 | 0.001984 |
| D\*-ImageNet | **0.6762** | 0.4379 | 0.030500 | 0.007892 |

- **再現は成立**: 折り A seed42 の COCO 塔は 0.724414 で凍結源 0.729749 と **−0.53 mAP**。
  分母参照の 3 seed 平均とは **0.70σ**
- test は **折りごとに一度、合計 10 回**（台帳に checkpoint の sha256 つきで記録）
- 14 run の所要は 7:48:59〜9:13:30、平均 **8.353 h**。同時実行は **2 本が最適**
  （1 本 1.805 step/s → 2 本 2.17 → 3 本 2.19 で頭打ち）

**予測の当たり外れ**: 1 当たり / 2 **部分的に外れ**（方向は 5/5 だが幅は 1.66〜7.33 mAP で
予測の 5〜15 に入るのは 2/5 折り）/ 3 当たり（16.3× と 3.9×）/ 4 **外れ**（最良 epoch は
COCO と同じ 11〜12 番目）。

**解釈**: 🔴 **D\*-ImageNet は 12 epoch で打ち切られている可能性が高い。** 最終 epoch が最良で、
最後の 1 epoch の伸びが D\*-COCO の **5.2 倍**（+0.0067 対 +0.0004）。予測 2 の差が小さかったのが
「ImageNet が強いから」か「両塔とも打ち切られているから」かは**切り分けていない（UNKNOWN）**。
H3（受け取り手の表現に依存するか）の材料に使う前に判断が要る。

もう一つ、**折り C・D・E では test mAP が val mAP を上回る**（折り E は val 0.5272 → test 0.5841）。
折りごとの難しさが val と test で逆向きに出ており、原因は調べていない。

**次**: 検出塔の学習の所要時間が代理 8.00 h から実測 8.35 h になり、計算器の UNKNOWN が
10 → 9 件へ減った。**Tier 1 の日数は +0.4% しか動かない**（12h/日・Stage 1 + Tier 1 で
160.5〜218.8 日）。代理の置き方が妥当だったことの確認になる。
