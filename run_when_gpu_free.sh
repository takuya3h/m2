...
# --- 実行 chain (Phase 3: TeCNO 学習 - 2 source × 3-seed × B2a/T1a) ----- #
run_tecno_training() {
  log "=== Phase 3: TeCNO 学習 (12 run: 2 source × 3-seed × B2a/T1a) ==="
  local vpy="$PROJECT_DIR/.venv/bin/python"
  cd "$PROJECT_DIR"
  # frozen source は 2 パターン (Relation-DETR seed42 / AlignDETR-S0-frozen seed42)。
  # train_b2a.py / train_t1a.py は FROZEN_SRC を module 定数で参照するので、環境変数で override
  # できるように改修されているかを確認。現状は relation_detr_seed42 固定なので、AlignDETR 側は
  # 別 wrapper script (train_b2a_aligndetr.py 等) or config で切替が必要。
  # このワンショット script では TODO とし、実装完了後に更新する。
  log "  TODO: 現状 train_b2a.py / train_t1a.py の FROZEN_SRC 定数を環境変数化 する PR が必要"
  log "  暫定: Relation-DETR 側のみ 3-seed 実行 (align 側は要実装後に別途)"
  for seed in 42 123 456; do
    log "  B2a Relation-DETR seed=$seed ..."
    CUDA_VISIBLE_DEVICES=0 "$vpy" scripts/train_b2a.py --seed "$seed" --epochs 50 \
      --description-override b2a_relationdetr_seed42frozen \
      > "$LOG_DIR/tecno_b2a_relation_seed${seed}.log" 2>&1
    log "  T1a Relation-DETR seed=$seed ..."
    CUDA_VISIBLE_DEVICES=0 "$vpy" scripts/train_t1a.py --seed "$seed" --epochs 50 \
      --description t1a_relationdetr_seed42frozen \
      > "$LOG_DIR/tecno_t1a_relation_seed${seed}.log" 2>&1
  done
  log "=== Phase 3 (Relation-DETR 側) 完了 ==="
}
