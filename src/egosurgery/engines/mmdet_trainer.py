...
class MMDetTrainer:
    ...
    def __init__(self, cfg) -> None:
        ...
        # === DDP 状態（setup() で確定）— §13.2 (b)(ii)・§8.0 条件 (5) ===
        # torchrun 起動時は WORLD_SIZE/RANK/LOCAL_RANK が環境変数で渡る。
        # mmdet Runner は dist 初期化・MMDistributedDataParallel ラップ・
        # SyncBatchNorm 変換を自動で行うため、本クラスは「DDP 文脈の検出」
        # と「eval_recipe への gpu_count 等の記録」だけを担う。
    ...
    def setup(self) -> None:
        ...
        DDP 文脈の検出も行う（§13.2 (b)(ii)）:
            torchrun は WORLD_SIZE / RANK / LOCAL_RANK 環境変数を設定する。
            mmdet Runner は自動で dist 初期化・DDP ラップを行うが、
            本トレーナーは eval_recipe に gpu_count 等を記録する必要があるため、
            ここで環境変数を読み取って self.world_size 等を確定する。
        """
        cfg = self.cfg
        seed_everything(int(cfg.seed))

        # --- DDP 環境の検出（§13.2 (b)(ii)・§8.0 条件 (4)(5)） ---
    ...
    def _build_eval_recipe(self) -> dict:
        ...
        per_gpu_bs = int(self.cfg.train.get("batch_size", 4))
        effective_bs = per_gpu_bs * max(1, self.world_size)
        return build_eval_recipe(
            test_cfg=test_cfg,
            split_sizes=split_sizes,
            server_name=self.server_name,
            gpu_count=max(1, self.world_size),
            effective_batch_size=effective_bs,
            lr_scaling=self._lr_scaling_label,
        )

    def _resolve_lr_label(self) -> str:
        """DDP の effective batch size に対する lr スケーリングラベルを返す。

        §8.0 条件 (6) で eval_recipe.lr_scaling に記録する文字列。
        cfg.train.lr_scaling_mode で運用方針を切り替える:
            - "linear": effective bs の倍率だけ lr を線形スケーリング
              → ラベル "linear_x{world_size}"（単一 GPU は "none"）
            - "keep_effective_bs": per-GPU bs を下げて effective bs を
              単一 GPU 時と同じに保ち lr 据え置き → "per_gpu_bs_adjusted"
        実際の lr 値の適用は mmdet の auto_scale_lr が batch 比例で行うため、
        本トレーナーはラベルのみ記録する（記録/実値の二重管理を避ける）。
        """
        if not self.is_distributed:
            return "none"
        mode = "linear"
        train_cfg = self.cfg.get("train", {}) if hasattr(self.cfg, "get") else {}
        if hasattr(train_cfg, "get"):
            mode = str(train_cfg.get("lr_scaling_mode", "linear"))
        if mode == "linear":
            return f"linear_x{self.world_size}"
        if mode == "keep_effective_bs":
            return "per_gpu_bs_adjusted"
        return mode
    ...
    def _init_wandb(self) -> None:
        ...
        §13.2 (b)(iii): DDP 時は rank=0 のみが W&B に書き込み（重複防止）。
        """
        if self.rank != 0:
            return
        ...
        if isinstance(wandb_config, dict):
            ...
            # DDP 構成を W&B config にも残し、後から rerun の整合性検証で参照できる。
            wandb_config["gpu_count"] = self.world_size
            wandb_config["effective_batch_size"] = int(
                self.cfg.train.get("batch_size", 4)
            ) * max(1, self.world_size)
            wandb_config["lr_scaling"] = self._lr_scaling_label
    ...
    def _resolve_detector(head_name: str) -> str:
        """detection_head 名から内部 detector キー（vfnet / dino / codetr）を決める。"""
