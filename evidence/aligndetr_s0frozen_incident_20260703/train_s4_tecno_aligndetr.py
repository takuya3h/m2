...
PROJ = Path("/home/ubuntu/slocal2/m2")
...
CACHE_DIR = PROJ / "data" / "processed" / "stage1_features" / "aligndetr_seed42"
...
def _desc(args) -> str:
    """証跡 description。aligndetr frozen source 版 (Entry 5 用)。"""
    base = "frozen_tecno_phase_baseline_aligndetr"
    if args.neck_from:
        return "frozen_tecno_phase_necktransfer_aligndetr"
    return base + ("_neck" if args.use_neck else "")
...
def _build_cfg(args, server_name: str, n_train: int, n_val: int) -> dict:
    ...
    return {
        ...
        "seed": args.seed,
        "frozen_source": {"detector": "align_detr", "seed": 42, "backbone": "resnet50",
